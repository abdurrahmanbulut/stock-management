...
import com.abdurrahmanbulut.sherlock.network.Service
...
class SplashRepository(private val api: SplashApi) {
    ...
    fun getFacts(coroutineScope: CoroutineScope, state: MutableState<ServiceResult<CatFact>>) {
        coroutineScope.call(state) {
            api.getFacts()
        }
    }

    fun getBreeds(coroutineScope: CoroutineScope, state: MutableState<ServiceResult<Breeds>>){
        coroutineScope.call(state) {
            api.getBreeds()
        }
    }
